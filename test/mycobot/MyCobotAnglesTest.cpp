...
int main(int argc, char *argv[])
{
  ...
  {
    // 2. 명령어 인자를 목표 각도로 변환
    mycobot::Angles target_angles;
    for (int i = 0; i < mycobot::Joints; ++i)
    {
    ...
    robot.WriteAngles(target_angles, 30); // 속도 80으로 이동
    ...
    for (int i = 0; i < 50; ++i) // 5초 동안 10Hz (100ms) 루프
    {
      ...
      mycobot::Angles current_angles = robot.PeekAngles();
      mycobot::IntAngles current_speeds = robot.PeekSpeeds();

      // c. 현재 상태 출력
      std::cout << "Time: " << std::setw(4) << i * 100 << "ms | ";
      std::cout << "Angles: [";
      for (size_t j = 0; j < current_angles.size(); ++j)
      {
        std::cout << std::setw(6) << std::fixed << std::setprecision(1) << current_angles[j] << (j < 5 ? "," : "");
      }
      std::cout << "] | Speeds: [";
